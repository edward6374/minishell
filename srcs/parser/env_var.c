...
/*   Updated: 2023/08/18 13:05:17 by nmota-bu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int other_chars(t_parser **list, t_vars *v)
{
	...
	//	printf("C: %c\tStart: %d\n", c, start);
	counter = 0;
	while ((c != '\0' && v->s[end] != c) || (c == '\0' && v->s[end] != c && v->s[end] != ' '))
	{
...
static char *take_name(t_vars *v)
{
	...
	while ((v->s[end] >= 'a' && v->s[end] <= 'z') || (v->s[end] >= 'A' && v->s[end] <= 'Z'))
		end++;
